          ...
          <div class="card-body">
            <div class="container-fluid p-0 pt-3">
              <div class="row align-items-center">
                <div class="col">
                  <div class="explainBoard"><strong>업체 관리입니다</strong></div>
                </div>
                <div class="col-auto">
            ...
            <table class="table text-center table-hover">
              <thead>
              <tr>
                <th scope="col" class="col-2">업체명</th>
                <th scope="col" class="col-5">주소</th>
                <th scope="col" class="col-2">신청일</th>
                <th scope="col" class="col-2">처리일</th>
...
<%-- 게시글              =======================================================================--%>
              <c:forEach items="${dtoList}" var="dto">
                <tr class="align-middle">
                  <th scope="row" class="text-truncate">${dto.name}</th>
                  <td class="text-start text-truncate">
                    <a href="/admin/shop/member/read/${dto.mno}" class="titleInList">${dto.address}</a>
                  </td>
                  <td>
                    <fmt:parseDate value="${dto.requestDate}" pattern="yyyy-MM-dd'T'HH:mm" var="parsedDateTime" type="both" />
                    <fmt:formatDate pattern="yy-MM-dd" value="${ parsedDateTime }" />
                  </td>
                  <td>
                    <fmt:parseDate value="${dto.approveDate}" pattern="yyyy-MM-dd'T'HH:mm" var="parsedDateTime" type="both" />
                    <fmt:formatDate pattern="yy-MM-dd" value="${ parsedDateTime }" />
                  </td>
                  <td>
                      ${state[dto.state]}
              ...
              <div class="row justify-content-md-center align-items-center">
                <div class="col col-auto">
                  <select class="selectType" aria-label="select search type">
                    <option value="w" ${(listDTO.type == null || listDTO.type == "w") ? 'selected' : ''}>업체명</option>
                    <option value="a" ${listDTO.type == "a" ? 'selected' : ''}>주소</option>
                    <option value="n" ${listDTO.type == "n" ? 'selected' : ''}>사업자번호</option>
                  </select>
                </div>
...
<form class="actionForm" action="/admin/shop/member" method="get">
...
<script>
    ...
    qsClickEvent(".pagination", (e) => {
        ...
        actionForm.setAttribute("action", "/admin/shop/member")
        actionForm.submit()
